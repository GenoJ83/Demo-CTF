...
- Exploit steps:
	...
	3. The page will show the Admin Panel and the flag: UCU{1d0r_vuln3r4bl3}.
...
- Exploit steps:
	...
	3. The app returns the debug view and the flag: UCU{d3s1gn_fl4w}.
...
- Exploit steps:
	...
	3. For this demo the script checks for that parameter and prints the flag: UCU{p4th_tr4v3rs4l} — you can also create a `flag.txt` file in the parent folder to simulate reading arbitrary files (do not do this on production systems).
...
- Exploit steps:
	...
	2. Click Test — the vulnerable `.html()` call executes the script, revealing the hidden flag element with UCU{xss_0ld_jq}.
...
- Exploit steps:
	...
	3. The script sets the session user to `admin` and shows the flag: UCU{4uth_f41l}.
...
- Exploit steps:
	...
	3. The app calculates MD5 and matches the stored hash, returning the flag: UCU{1nt3gr1ty_br34k}.

9) a09_logging_failures.php — No logging / brute-force demo (PHP)
- Run: start the PHP built-in server.
- Exploit steps:
	1. Visit http://localhost:8000/a09_logging_failures.php
	2. Submit the form with `attempt=admin` (or post to the endpoint) — the script will return the flag: UCU{l0gg1ng_m1ss} (the point: no logging/rate-limit in place).
...
- Exploit steps:
	...
	3. The demo checks for that IP and returns the metadata flag: UCU{ssrf_4tt4ck} (do NOT perform SSRF against real metadata endpoints on cloud infrastructure unless you control the environment).
